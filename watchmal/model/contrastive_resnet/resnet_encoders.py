...
class ResNetEncoder(nn.Module):
    ...
    def __init__(self, block, layers, first_conv=False, maxpool1=False,image_channels=1):
        super().__init__()
        ...
        if self.first_conv:
            self.conv1 = nn.Conv2d(
                self.image_channels, self.inplanes, kernel_size=7, stride=2, padding=3, bias=False
            )
        else:
            self.conv1 = nn.Conv2d(
                self.image_channels, self.inplanes, kernel_size=3, stride=1, padding=1, bias=False
            )
...
def resnet18_encoder(first_conv, maxpool1,img_channels):
    return ResNetEncoder(EncoderBlock, [2, 2, 2, 2], first_conv, maxpool1,img_channels)
...
def resnet50_encoder(first_conv, maxpool1,img_channels):
    return ResNetEncoder(EncoderBottleneck, [3, 4, 6, 3], first_conv, maxpool1,img_channels)
