...
#Main evaluation loop
    ...
    def test_step(self):
        x = self.data.to(self.device)
        y = self.labels.to(self.device)
        mu= self.model.fc_mu(self.model.encoder(x))
        z = mu
        l_features = [torch.flatten(self.model.encoder.get_layer_output(x,i),1) for i in range(1,5)] + [mu]
