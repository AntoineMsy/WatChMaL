...
class ClassifierEngine:
    ...
    def evaluate(self, test_config):
        ...
        with torch.no_grad():
            ...
            loss, accuracy, indices, labels, predictions=  [],[],[],[],[]
            softmaxes = {}
            ...
            for it, eval_data in enumerate(self.data_loaders["test"]):
                ...
                keys = eval_data["root_file"]
                for i in range(len(keys)):
                    if keys[i] not in softmaxes.keys():
                        softmaxes[keys[i]] = []        
                    softmaxes[keys[i]].append(result["softmax"][i].detach().cpu().numpy())
        ...
        if self.is_distributed:
            ...
            if self.rank == 0:
                ...
                print(type(global_eval_results_dict["softmaxes"].cpu()))
                for key in global_eval_results_dict["softmaxes"].cpu() : 
                    print("key is " + key)
                    np.save(self.dirpath + "softmax%s.npy"%(key), np.array(global_eval_results_dict["softmaxes"].cpu()[key]))#[sorted_indices])
...
#            sorted_indices = np.argsort(indices)
            ...
            for key in softmaxes : 
                print(self.dirpath + "softmax%s.npy"%(key), np.array(softmaxes[key]))
                np.save(self.dirpath + "softmax%s.npy"%(key), np.array(softmaxes[key]))#[sorted_indices])
