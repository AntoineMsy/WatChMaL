...
class CNNmPMTEventDisplay(CNNmPMTDataset):
    ...
    """
    def plot_data_2d(self, data, label = None, transformations=None, **kwargs):
        """
        ...
        return plot_event_2d(data_nan.flatten(), coordinates, mpmt_coordinates, label, **kwargs)

    def plot_event_2d(self, event, transformations=None, log = False, **kwargs):
        """
        ...
        return self.plot_data_2d(data, label, transformations=transformations, **kwargs)

    def plot_processed_event_2d(self, event, model, transformations=None, log = False, **kwargs):
        """
        Plots an event decoded by the SAE
        """
        data = self.pad(torch.Tensor(self[event]['data']))[None,:]
        if log : 
            data = self.log_transform(data)
        out , y = model(data)
        out = self.unpad(out.squeeze(0))
        #mask = out >= 0.5
        #new = torch.zeros(out.size())
        #new[mask] = out[mask] 
        #out = new.detach().numpy()
        out = out.detach().numpy()
        return self.plot_data_2d(out, transformations=transformations, **kwargs)

    def plot_diff_2d(self, event, model, transformations=None, **kwargs):
        """
        Plots an event decoded by the SAE
        """
        data = self.pad(torch.Tensor(self[event]['data']))[None,:]
        out , y = model(data)
        out = self.unpad(out.squeeze(0))
        diff = out-self[event]['data']
        print(torch.min(diff))
        print(torch.max(diff))
        diff = diff.detach().numpy()
        
        return self.plot_data_2d(diff, transformations=transformations, **kwargs)

    def plot_generated_event_2d(self, event, model, transformations=None, log = False, **kwargs):
        """
        Plot an event generated by the SAE
        """
        data = self.pad(self[event]['data'])[None,:]
        if log:
            data = self.log_transform(data)
        label = self[event]["labels"]
