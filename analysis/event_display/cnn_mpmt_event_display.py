...
class CNNmPMTEventDisplay(CNNmPMTDataset):
    ...
    def plot_processed_event_2d(self, event, model, transformations=None, **kwargs):
        """
        Plots an event decoded by the SAE
        """
        data = self.pad(torch.Tensor(self[event]['data']))[None,:]
        out , y = model(data)
        out = self.unpad(out.squeeze(0)).detach().numpy()
        return self.plot_data_2d(out, transformations=transformations, **kwargs)

    
    def plot_generated_event_2d(self, event, model, transformations=None, **kwargs):
        """
        ...
        out = self.unpad(example.squeeze(0)).detach().numpy()
        return self.plot_data_2d(out, transformations=transformations, **kwargs)
